  ...
  useEffect(
    function () {
      async function getFacts() {
        ...
        const { data: facts, error } = await query.order("text", { ascending: true }).limit(100);
...
function FactLists({ facts, setFacts }) {
  if (facts.length === 0) {
  ...
  return (
    <section>
      ...
      <p>The are {facts.length} facts in the database. Add your own</p>
    </section>
  );
}
